...
public class WidgetDao{
	...
	public int createWidgetForPage(int pageId, Widget widget){
        ...
        try{
            ...
            String query = "INSERT INTO widget (id, name, width, height, cssClass, cssStyle, text, "
    					 + "`order`, pageId, type, url, sharable, expandable, src, size, html)"
    					 + " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public Collection<Widget> findAllWidgets(){
        ...
        try{
            ...
            while(rs.next()){  
                ...
                switch(type){
                    case "heading":
                        widget = new HeadingWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"), rs.getInt("size"));
                        break;
                    case "html":
                        widget = new HtmlWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"), rs.getString("html"));
                        break;
                    case "youtube":
                        widget = new YouTubeWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"), rs.getString("src"), 
                        		rs.getBoolean("sharable"), rs.getBoolean("expandable"));
                        break;
                    case "image":
                        widget = new ImageWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"), rs.getString("url"));
                        break;
                    default:
                        widget = new Widget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"));
                        break;
                }
                widgets.add(widget);
			}
            ...
            while(rs.next()){
                ...
                switch(type){
                    ...
                    case "youtube":
                        widget = new YouTubeWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
                        		rs.getString("text"), rs.getInt("order"), rs.getString("src"), rs.getBoolean("sharable"), rs.getBoolean("expandable"));
                        break;
                    case "image":
                        widget = new ImageWidget(rs.getInt("id"), rs.getString("name"), rs.getInt("width"), 
                        		rs.getInt("height"), rs.getString("cssClass"), rs.getString("cssStyle"), 
	...
	public int updateWidget(int widgetId, Widget widget){
        ...
        try{
            Class.forName(BaseDao.DRIVER);
            conn = DriverManager.getConnection(BaseDao.URL, BaseDao.USERNAME, BaseDao.PASSWORD);
			String query = "UPDATE widget SET name = ?, width = ?, height = ?, cssClass = ?, cssStyle = ?, "
						 + "text = ?, `order` =?, type = ?, url = ?, sharable = ?, expandable = ?, src = ?, "
						 + "size = ?, html = ? WHERE id = ?";
            statement = conn.prepareStatement(query);
