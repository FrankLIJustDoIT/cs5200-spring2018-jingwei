...
public class DeveloperDao{
	...
	public int createDeveloper(Developer developer){
		...
		try{
			...
			String query1 = "INSERT INTO person(id, username, password, firstName, lastName, email, dob)"
						+ " VALUES(?,?,?,?,?,?,?)";
	...
	public Collection<Developer> findAllDevelopers(){
        ...
        try{
			...
			while(rs.next()){
                Developer developer = new Developer(rs.getInt("id"), rs.getString("username"), 
            							rs.getString("password"),rs.getString("firstName"),
            							rs.getString("lastName"), rs.getString("email"),rs.getDate("dob"),
            							rs.getString("developerKey"));
	...
	public Developer findDeveloperById(int developerId){
        ...
        try{
            Class.forName(BaseDao.DRIVER);
            conn = DriverManager.getConnection(BaseDao.URL, BaseDao.USERNAME, BaseDao.PASSWORD);
			String query = "select p.*, d.* from person p inner join developer d on p.id = d.id"
						+ " where p.id = ?";
			statement = conn.prepareStatement(query);
        ...
        try{
			...
			while(rs.next()){
                Developer developer = new Developer(rs.getInt("id"), rs.getString("username"), 
                		...
                		rs.getString("email"),rs.getDate("dob"),rs.getString("developerKey"));
                developer.setWebsites(WebsiteDao.getInstance().findWebsitesForDeveloper(developer.getId()));
                dev = developer;
            }
		}catch (ClassNotFoundException e){
	...
	public int updateDeveloper(int developerId, Developer developer){
        ...
        try{
            Class.forName(BaseDao.DRIVER);
            conn = DriverManager.getConnection(BaseDao.URL, BaseDao.USERNAME, BaseDao.PASSWORD);
			String query = "UPDATE person p, developer d SET p.username=?, p.password=?, p.firstName=?,"
						+ " p.lastName=?, p.email=?, p.dob=?, d.developerKey=? WHERE p.id=? AND p.id=d.id";
			statement = conn.prepareStatement(query);
    ...
    public int updatePhoneForDeveloper(int developerId, Phone phone){
        ...
        try{
            ...
            statement.setBoolean(2, phone.getPrimary());
            statement.setBoolean(3, phone.getPrimary());
